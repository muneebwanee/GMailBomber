...
## Note:
* ### This only works with an `gmail account` (so emails are not going in spam folder)

## Installation :
* Go to this link: https://myaccount.google.com/lesssecureapps?pli=1 And make it gray so the program can send
the emails.

* `git clone https://github.com/muneebwanee/GMailBomber.git`
* `cd GMailBomber`
* >Run: `python bomber.py`

* Enter email, enter password (password will not be shown in program), enter the email you want to
bomb, enter message and then enter amount of emails to be sent.

## Single Command :
```
git clone https://github.com/muneebwanee/GMailBomber.git ; cd GMailBomber ; python bomber.py
```
